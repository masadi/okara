...
class PenilaianController extends Controller
{
        ...
        foreach(request()->kompeten as $uuid => $kompeten){
            ...
            if($tp){
                ...
                if($kompeten){
                    ...
                    Tp_nilai::where(function($query) use ($tp_id, $anggota_rombel_id){
                        $query->where('sekolah_id', request()->sekolah_id);
                        $query->where('anggota_rombel_id', $anggota_rombel_id);
                        $query->where('tp_id', $tp_id);
                    })->delete();
                }
            }
        }
    ...
    public function get_ekskul(){
        $data = [
            'data_ekskul' => Ekstrakurikuler::with(['sekolah' => function($query){
                $query->select('sekolah_id', 'nama');
            }])->where('guru_id', request()->guru_id)->where('semester_id', request()->semester_id)->get(),
            'data_nilai' => collect([
                [
                    'id' => 1,
                    'name' => 'Sangat Baik'
    ...
    public function hapus_sikap(){
        $find = Nilai_budaya_kerja::find(request()->sikap_id);
        if($find->delete()){
            $data = [
                'icon' => 'success',
                'title' => 'Berhasil!',
                'text' => 'Nilai Sikap berhasil dihapus',
            ];
        } else {
            $data = [
                'icon' => 'error',
                'title' => 'Gagal!',
                'text' => 'Nilai Sikap gagal dihapus. Silahkan coba beberapa saat lagi!',
            ];
        }
        return response()->json($data); 
    }
