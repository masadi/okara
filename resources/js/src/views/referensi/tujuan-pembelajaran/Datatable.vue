    ...
    <b-overlay :show="loading" rounded opacity="0.6" size="lg" spinner-variant="warning">
      <b-table responsive bordered striped :items="items" :fields="fields" :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" show-empty :busy="isBusy">
        ...
        <template v-slot:cell(kelas)="row">
          ...
          <template v-else>
            <span v-if="row.item.kd.kelas_1">1</span>
            <span v-if="row.item.kd.kelas_2">2</span>
            <span v-if="row.item.kd.kelas_3">3</span>
            <span v-if="row.item.kd.kelas_4">4</span>
            <span v-if="row.item.kd.kelas_5">5</span>
            <span v-if="row.item.kd.kelas_6">6</span>
            <span v-if="row.item.kd.kelas_7">7</span>
            <span v-if="row.item.kd.kelas_8">8</span>
            <span v-if="row.item.kd.kelas_9">9</span>
