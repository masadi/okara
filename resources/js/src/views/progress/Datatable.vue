    ...
    <b-overlay :show="loading" rounded opacity="0.6" size="lg" spinner-variant="warning">
      <b-table responsive bordered striped :items="items" :fields="fields" :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" show-empty :busy="isBusy">
        ...
        <template v-slot:cell(detil_rombongan_belajar_id)="row">
          <b-button variant="success" size="sm" @click="getDetilEkskul(row.item.rombongan_belajar_id, row.item.kelas_ekskul.ekstrakurikuler_id)">Detil</b-button>
        </template>
      </b-table>
    </b-overlay>
    <b-row class="mt-2">
...
export default {
  ...
  methods: {
    getDetil(pembelajaran_id){
      this.$emit('detil', pembelajaran_id)
    },
    getDetilEkskul(rombongan_belajar_id, ekstrakurikuler_id){
      this.$emit('detil', {
